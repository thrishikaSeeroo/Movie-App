...
import 'package:the_movie_app/features/movie/presentation/widgets/horizontal_all_movie_list.dart';
...
class _MovieListScreenState extends State<MovieListScreen> {
  ...
  void initState() {
    context
        ...
        .add(const GetMovieListEvent.fetch(pageNo: 1));
    context
        .read<FavoriteMoviesBloc>()
        .add(const FavoriteMoviesEvent.fetch(accountId: 21773962));
    context
        .read<WatchlistBloc>()
        .add(const WatchlistEvent.fetch(accountId: 21773962));
    super.initState();
  }
                      ...
                      BlocBuilder<FavoriteMoviesBloc, FavoriteMoviesState>(
                        builder: (context, state) {
                          return state.when(
                              ...
                              success: (data) {
                                final List<Result> favMovieList =
                                    data.results ?? [];
                                return Container(
                                  color: Colors.white,
                                  ...
                                  child: HorizontalListWidget(
                                    ...
                                    moviesList: favMovieList,
                                  ),
                                );
                              });
                        },
                      ),
                      //pagination added in movies list
                      HorizontalAllMovieListWidget(
                        title: 'Movies',
                        moviesList: [],
                      ),
